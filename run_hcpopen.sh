...
while read p; do
    if [[ "${exclIDs[@]}" =~ "${p}" ]]; then
        ...
        spark-submit --jars jars/aws-java-sdk-1.7.4.jar,jars/hadoop-aws-2.7.7.jar --master spark://m5al0:7077 --total-executor-cores 2 --executor-memory 1G examples/src/main/python/pipeline_hcpopen.py -b hcp-openaccess -n ${p} -s hcpopen -l /tmp/pipeline_hcpopen_${p}.log >& /tmp/${p}.log &
        sleep 1
    fi
done < $filelist
