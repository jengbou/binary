...
while read p; do
    if [[ "${exclIDs[@]}" =~ "${p}" ]]; then
        ...
        spark-submit --jars jars/aws-java-sdk-1.7.4.jar,jars/hadoop-aws-2.7.7.jar --master spark://m5a2x0:7077 --total-executor-cores 2 --executor-memory 1G examples/src/main/python/pipeline_hcpopen_mvp.py -b hcp-openaccess -n ${p} -s hcpopen >& /tmp/${p}.log &
        sleep 1
    fi
done < $filelist
